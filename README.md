...
- **Cursor Cloud agents:** cannot call GitHub PR APIs on the built-in token (`updatePullRequest` blocked). **Fix:** add `GH_PAT` Runtime Secret to Cloud Environment — see [docs/cursor-github-integration.md](docs/cursor-github-integration.md). **Fallback:** push to `cursor/**` + [Cursor Agent Auto Ship](.github/workflows/cursor-agent-auto-ship.yml)
